// ========================================================
// CHIEF HUMAN RESOURCES OFFICER (CHRO) LOGIC
// ========================================================

function initChroDashboard() {
    renderChroStats();
    renderChroProvidersTable();
}

// Compute and render stats cards
function renderChroStats() {
    ...
    document.getElementById("chro-total-partners").textContent = total;
    document.getElementById("chro-purohit-count").textContent = `${purohits} Purohits`;
    document.getElementById("chro-pending-approvals").textContent = pending;
    document.getElementById("chro-approved-partners").textContent = approved;
    document.getElementById("chro-suspended-partners").textContent = suspended;
}

// Render partners vetting queue table
function renderChroProvidersTable() {
    const tableBody = document.getElementById("chro-providers-table");
    const providers = db.getProviders();
    tableBody.innerHTML = "";
...
function inspectProvider(provId) {
    ...
    const modalContent = document.getElementById("chro-modal-inspector-content");
    ...
    modalContent.innerHTML = `
        ...
        <div style="display:flex; justify-content:flex-end; gap:10px; border-top:1px solid var(--border-color); padding-top:15px; margin-top:1.5rem;">
            <button class="btn btn-secondary" onclick="closeModal('chro-inspector-modal')">Close Audit</button>
            ${p.status === 'pending' ? `
                <button class="btn btn-primary" style="background:var(--success); color:#FFF;" onclick="updateProviderStatus('${p.id}', 'approved'); closeModal('chro-inspector-modal');">Approve Partner</button>
                <button class="btn btn-danger" onclick="updateProviderStatus('${p.id}', 'suspended'); closeModal('chro-inspector-modal');">Reject Partner</button>
            ` : ''}
        </div>
    `;

    openModal("chro-inspector-modal");
}
...
function updateProviderStatus(provId, nextStatus) {
    ...
    showNotification(`💼 CHRO Action: ${partnerName} has been ${actionLabel}!`);
}
